...
gs <- factor(sml)
groups <- make.names(c("Control","Clone9"))
levels(gs) <- groups
gset$group <- gs
...
clone9_tt <- subset(tT, rownames(tT) %in% clone9_circs)
write.table(clone9_tt, "/data/github/pca_network/results/GSE118959_Clone9_limma.txt", sep="\t", row.names = F, quote=F)
