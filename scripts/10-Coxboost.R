...
coxboost_preds <- schoenfeld_result_app$preds
...
dev_mat$group <- ifelse(dev_mat$lp > median(dev_mat$lp), "High", "Low")
fit_dev <- survfit(Surv(days_to_follow_up, bcr_status) ~ group, data = dev_mat)
surv_object <- Surv(dev_mat$days_to_follow_up, dev_mat$bcr_status)
res2 = survfit(surv_object ~ group, data=dev_mat)
p1 <- ggsurvplot(res2,
                 pval = F, conf.int = F,
...
p1$plot <- p1$plot + 
  ggplot2::annotate(
    ...
    x=400, y = 0.1,
    #vjust=3, hjust=1,
    label = "HR = 1.385 (1.15-1.62)\np < 0.0001",
    size = 5
  )

p1

pdf(p1, file="/data/github/pca_network/results/2025-07-01/kaplan_master.pdf", height = 8, width=10)
p1
dev.off()
...
pdf("/data/github/pca_network/results/2025-07-01/master_ROC.pdf", width=7, height=6)
...
ext_mat$group <- ifelse(ext_mat$lp > median(ext_mat$lp), "High", "Low")
fit_ext <- survfit(Surv(days_to_follow_up, bcr_status) ~ group, data = ext_mat)
surv_object <- Surv(ext_mat$days_to_follow_up, ext_mat$bcr_status)
res2 = survfit(surv_object ~ group, data=ext_mat)
p1 <- ggsurvplot(res2,
                 pval = F, conf.int = F,
                 risk.table = T, # Add risk table
                 #risk.table.col = "strata", # Change risk table color by groups
...
p1$plot <- p1$plot + 
  ggplot2::annotate(
    ...
    x=300, y = 0.1,
    #vjust=3, hjust=1,
    label = "HR = 1.479 (0.52-2.44)\np = 0.0011",
    size = 5
  )

p1

pdf(p1, file="/data/github/pca_network/results/2025-07-01/kaplan_external.pdf", height = 8, width=10)
p1
dev.off()
...
pdf("/data/github/pca_network/results/2025-07-01/external_ROC.pdf", width=7, height=6)
plot(ROC.1, time=365, title = F, lwd=2)
plot(ROC.2, time=floor(365*3), col="blue", add = T, title=F, lwd=2)
plot(ROC.3, time=floor(365*5), col="forestgreen", add = T, title=F, lwd=2)
my_legend =  c(paste0("1 year AUC: ", round(ROC.1$AUC[2],2), "; 95% CI: ", paste(as.numeric(confint(ROC.1)$CI_AUC), collapse = "-")),
...
# ca;ibration plot?
dat <- mat[keep_patient, c(names(coxboost_preds), "days_to_follow_up", "bcr_status")]
d = rms::datadist(dat)
options(datadist="d")
units(d) = "Day"
form <- as.formula(paste("Surv(days_to_follow_up, bcr_status) ~", paste(names(coxboost_preds), collapse = " + ")))
rms_cox1 = rms::cph(form, data=dat, init=as.numeric(coxboost_preds), iter.max = 0, surv = T, x=T, y=T, time.inc = 356)
rms_cox3 = rms::cph(form, data=dat, init=as.numeric(coxboost_preds), iter.max = 0, surv = T, x=T, y=T, time.inc = 356*3)
rms_cox5 = rms::cph(form, data=dat, init=as.numeric(coxboost_preds), iter.max = 0, surv = T, x=T, y=T, time.inc = 356*5)

cal1yr <- calibrate(rms_cox1, B=500, u=365*1, conf.int=TRUE, cmethod = "KM", what = "observed")
cal3yr <- calibrate(rms_cox3, B=500, u=365*3, conf.int=TRUE, cmethod = "KM", what = "observed")
cal5yr <- calibrate(rms_cox5, B=500, u=365*5, conf.int=TRUE, cmethod = "KM", what = "observed")

pdf("/data/github/pca_network/results/2025-07-01/1yrcalibration.pdf", width=7, height=7)
plot(cal1yr, xlim=c(0.5,1), ylim=c(0.5,1), col="black", add=F, lwd=1.5)
dev.off()
pdf("/data/github/pca_network/results/2025-07-01/3yrcalibration.pdf", width=7, height=7)
plot(cal3yr,  xlim=c(0.3,1), ylim=c(0.3,1), col="black", add=F, lwd=1.5)
dev.off()
pdf("/data/github/pca_network/results/2025-07-01/5yrcalibration.pdf", width=7, height=7)
plot(cal5yr,  xlim=c(0.1,1), ylim=c(0.1,1), col="black", add=F, lwd=1.5)
dev.off()

my_legend =  c(paste0("1 year"),
               paste0("3 year"),
               paste0("5 year"))
legend("bottomright", legend = my_legend,col=c("forestgreen", "blue", "red"),lwd=2, cex=1)
dev.off()
