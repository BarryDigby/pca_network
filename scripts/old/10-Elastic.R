...
alpha_seq = seq(0.1, 1, length = 20)
lambda_max <- max(glmnet(x, y, family = "cox", alpha = 1)$lambda)
# stretch lambda max to return sparser model
lambda_seq <- exp(seq(log(lambda_max * 4), log(lambda_max * 2), length = 50))

fitEN <- list()
for (i in 1:length(alpha_seq)) {
  fitEN[[i]] <- cv.glmnet(x, y, family = "cox", alpha = alpha_seq[i], lambda = lambda_seq, foldid = group_identifier)
}
...
elastic_preds <- coef(elastic_fit$glmnet.fit, s=elastic_fit$lambda.1se)[,1]
elastic_preds <- elastic_preds[elastic_preds != 0]

# explore number of non-zero genes returned by each lambda
#nz_counts <- sapply(1:length(elastic_fit$glmnet.fit$lambda), function(i) {
#  sum(coef(elastic_fit$glmnet.fit, s = elastic_fit$glmnet.fit$lambda[i]) != 0)
#})
#names(nz_counts) <- round(elastic_fit$glmnet.fit$lambda, 4)

## function to remove offending predictors (schoenfeld residuals.)
remove_offending_predictors <- function(mat, preds) {
  repeat {
    # Create the formula
    fx <- formula(paste("Surv(days_to_follow_up, bcr_status) ~", paste(names(preds), collapse = " + ")))
    
    # Fit the Cox proportional hazards model with lasso coefficients
    fit_zph <- cox.zph(coxph(fx, data = mat, init = as.numeric(preds), iter.max = 0))
    ...
    preds <- preds[!(names(preds) %in% remove)]
  ...
  fit <- coxph(fx, data = mat, init = as.numeric(preds), iter.max = 0)
  
  return(list(fit = fit, preds = preds))
}

schoenfeld_result <- remove_offending_predictors(mat, elastic_preds)

elastic_preds <- schoenfeld_result$preds
...
elastic_master_result <- data.frame()
dev_mat <- mat
...
form <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(names(elastic_preds), collapse = "+"), " + offset(lp)"))
fit_dev <- coxph(formula = form, data = dev_mat, init = c(as.numeric(elastic_preds, 1)), iter.max=0)
...
elastic_master_result <- rbind(elastic_master_result, t(as.data.frame(out_dev)))

colnames(elastic_master_result) <- c(
...
elastic_master_result <- elastic_master_result%>%
  mutate(Dataset = "master_dataset")%>%
  dplyr::select(Dataset, everything())

write.table(elastic_master_result, file = paste(outdir, "/elastic_master.txt", sep=""), row.names = F, quote=F, sep="\t")
...
elastic_iecv_result <- data.frame()

for(i in unique(mat$study)){
  print(i)
  dev_mat <- mat %>% dplyr::filter(!(study %in% i))
  val_mat <- mat %>% dplyr::filter(study %in% i)
  dev_mat <- dev_mat[, c(names(elastic_preds), "days_to_follow_up", "bcr_status")]
  val_mat <- val_mat[, c(names(elastic_preds), "days_to_follow_up", "bcr_status")]
  
  # construct coxph model - use lasso coeficients
  fx <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(names(elastic_preds), collapse = " + ")))
  dev_fit <- coxph(fx, dev_mat, init = as.numeric(elastic_preds), iter.max=0)
  val_fit <- coxph(fx, val_mat, init = as.numeric(elastic_preds), iter.max=0)
  ...
  form <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(names(elastic_preds), collapse = "+"), " + offset(lp)"))
  fit_dev <- coxph(formula = form, data = dev_mat, init = c(as.numeric(elastic_preds, 1)), iter.max=0)
  fit_val <- coxph(formula = form, data = val_mat, init = c(as.numeric(elastic_preds, 1)), iter.max=0)
  ...
  elastic_iecv_result <- rbind(elastic_iecv_result, t(as.data.frame(out_dev)))
  rownames(elastic_iecv_result)[nrow(elastic_iecv_result)] <- paste0(i, "_dev")
  elastic_iecv_result <- rbind(elastic_iecv_result, t(as.data.frame(out_val)))
  rownames(elastic_iecv_result)[nrow(elastic_iecv_result)] <- paste0(i, "_val")
}

colnames(elastic_iecv_result) <- c(
  "Slope Coef",
  "Slope LCI",
  "Slope UCI",
  "Slope Pval",
...
elastic_iecv_result <- elastic_iecv_result%>%
  tibble::rownames_to_column(var = "Dataset")%>%
  dplyr::select(Dataset, everything())

write.table(elastic_iecv_result, file = paste(outdir, "/elastic_iecv.txt", sep=""), row.names = F, quote=F, sep="\t")
...
elastic_iecv_result$info <- rep(res, each=2)
...
forest_plot <- function(model, effect, LCI, UCI, rlab){
  ...
  df <- elastic_iecv_result %>% tibble::rownames_to_column(., "rownames") %>% dplyr::filter(grepl(paste0(model), rownames))
...
pdf(file = paste(outdir, "/elastic_iecv_slope.pdf", sep=""), width = 8, height = 6)
forest_plot("_val", "Slope Coef", "Slope LCI", "Slope UCI", "Calibration Slope")
dev.off()

pdf(file = paste(outdir, "/elastic_iecv_itl.pdf", sep=""), width = 8, height = 6)
forest_plot("_val", "Slope ITL Coef", "Slope ITL LCI", "Slope ITL UCI", "Calibration ITL")
dev.off()

pdf(file = paste(outdir, "/elastic_iecv_harrells.pdf", sep=""), width = 8, height = 6)
...
ext_mat <- ext_mat[, c(names(elastic_preds), "days_to_follow_up", "bcr_status")]

# External, unseen dataset results
fx <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(names(elastic_preds), collapse = " + ")))
ext_fit <- coxph(fx, data = ext_mat, init = as.numeric(elastic_preds), iter.max=0)
...
form <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(names(elastic_preds), collapse = "+"), " + offset(lp)"))
fit_ext <- coxph(formula = form, data = ext_mat, init = c(as.numeric(elastic_preds, 1)), iter.max=0)
...
elastic_ext_val <- rbind(elastic_ext_val, t(as.data.frame(out_val)))

colnames(elastic_ext_val) <- c(
  "Slope Coef",
  "Slope LCI",
  "Slope UCI",
  "Slope Pval",
...
elastic_ext_val <- elastic_ext_val%>%
  mutate(Dataset = "GSE46602_ext")%>%
  dplyr::select(Dataset, everything())

write.table(elastic_ext_val, file = paste(outdir, "/elastic_external.txt", sep=""), row.names = F, quote=F, sep="\t")

source_file <- "/data/github/pca_network/scripts/10-Elastic.R"
