...
outdir <- paste(outdir, Sys.Date() + 1, sep="")
if(!dir.exists(outdir)){
  dir.create(outdir)
}

## Intersect predictors from LASSO, CoxBoost and Elastic net
lasso <- read.table("/data/github/pca_network/results/2025-06-24/lasso_external.txt", sep="\t", header=T)
lasso <- trimws(unlist(strsplit(lasso$genes, ",")))
elastic <- read.table("/data/github/pca_network/results/2025-06-24/elastic_external.txt", sep="\t", header=T)
elastic <- trimws(unlist(strsplit(elastic$genes, ",")))
boost <- read.table("/data/github/pca_network/results/2025-06-24/coxboost_external.txt", sep="\t", header=T)
boost <- trimws(unlist(strsplit(boost$genes, ",")))
pred_genes <- c(lasso, boost, elastic)
pred_counts <- table(pred_genes)
three_model <- names(pred_counts[pred_counts >= 3])
two_model <- names(pred_counts[pred_counts >=2])
one_model <- names(pred_counts[pred_counts >=1])

model_genes <- c("one_model", "two_model", "three_model")

for (model in model_genes){
    
  genes <- get(model)
  
  load("/data/github/pca_network/results/processed_datasets.RData")
  mat <- master_mat[, c(genes, "days_to_follow_up", "bcr_status", "study")]
  rm(master_mat)
  formula_string <- paste(paste(genes, collapse = " + "), "+ (1 | study)")
  coxme_formula <- as.formula(paste("Surv(days_to_follow_up, bcr_status) ~", formula_string))
  coxme_fit <- coxme::coxme(coxme_formula, data = mat)
  coxph_mat <- mat %>% dplyr::select(-study)
  # coxboost_preds referencd a lot in the script, define it here
  # use coefficients from coxme - as uses study as random effect.
  fit <- coxph(Surv(days_to_follow_up, bcr_status) ~ ., init=as.numeric(coxme_fit$coefficients), iter.max=0, data=coxph_mat)
  
  coxboost_preds <- coef(fit)
  
  # STEP 2
  
  intersect_master_result <- data.frame()
  dev_mat <- coxph_mat
  ...
  intersect_master_result <- rbind(intersect_master_result, t(as.data.frame(out_dev)))
  
  colnames(intersect_master_result) <- c(
  ...
  intersect_master_result <- intersect_master_result%>%
    mutate(Dataset = "master_dataset")%>%
    dplyr::select(Dataset, everything())
  
  write.table(intersect_master_result, file = paste(outdir, "/", model, "_master.txt", sep=""), row.names = F, quote=F, sep="\t")
  ...
  intersect_iecv_result <- data.frame()
  ...
  for(i in unique(mat$study)){
    ...
    intersect_iecv_result <- rbind(intersect_iecv_result, t(as.data.frame(out_dev)))
    rownames(intersect_iecv_result)[nrow(intersect_iecv_result)] <- paste0(i, "_dev")
    intersect_iecv_result <- rbind(intersect_iecv_result, t(as.data.frame(out_val)))
    rownames(intersect_iecv_result)[nrow(intersect_iecv_result)] <- paste0(i, "_val")
  }
  
  colnames(intersect_iecv_result) <- c(
    "Slope Coef",
    "Slope LCI",
    "Slope UCI",
    "Slope Pval",
  ...
  intersect_iecv_result <- intersect_iecv_result%>%
    tibble::rownames_to_column(var = "Dataset")%>%
    dplyr::select(Dataset, everything())
  
  write.table(intersect_iecv_result, file = paste(outdir, "/", model, "_iecv.txt", sep=""), row.names = F, quote=F, sep="\t")
  ...
  intersect_iecv_result$info <- rep(res, each=2)
  ...
  forest_plot <- function(model, effect, LCI, UCI, rlab){
    ...
    df <- intersect_iecv_result %>% tibble::rownames_to_column(., "rownames") %>% dplyr::filter(grepl(paste0(model), rownames))
  ...
  pdf(file = paste(outdir, "/", model, "_iecv_slope.pdf", sep=""), width = 8, height = 6)
  forest_plot("_val", "Slope Coef", "Slope LCI", "Slope UCI", "Calibration Slope")
  dev.off()
  
  pdf(file = paste(outdir, "/", model, "_iecv_itl.pdf", sep=""), width = 8, height = 6)
  forest_plot("_val", "Slope ITL Coef", "Slope ITL LCI", "Slope ITL UCI", "Calibration ITL")
  dev.off()
  
  pdf(file = paste(outdir, "/", model, "_iecv_harrells.pdf", sep=""), width = 8, height = 6)
  ...
  intersect_ext_val <- rbind(intersect_ext_val, t(as.data.frame(out_val)))
  
  colnames(intersect_ext_val) <- c(
    "Slope Coef",
    "Slope LCI",
    "Slope UCI",
    "Slope Pval",
  ...
  intersect_ext_val <- intersect_ext_val%>%
    mutate(Dataset = "GSE46602_ext")%>%
    dplyr::select(Dataset, everything())
  
  write.table(intersect_ext_val, file = paste(outdir, "/", model, "_external.txt", sep=""), row.names = F, quote=F, sep="\t")

}

source_file <- "/data/github/pca_network/scripts/10-Intersection.R"
