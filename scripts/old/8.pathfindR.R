...
input = network[,c(5,6)]
input$adj.P.val = 0.001 # used for filtering >  0.05 in pathfindr - rnas have already been filtered. 
colnames(input) = c("Gene.Symbol", "logFC", "adj.P.Val")
input = input %>% unique()

# Run analysis and save. load for future sessions. 
keggpathway = pathfindR::run_pathfindR(input, gene_sets = "KEGG", plot_enrichment_chart = F, list_active_snw_genes = T, n_processes=6, iterations=100)
write.table(keggpathway, "/data/github/pca_network/results/kegg.txt", quote = F, row.names = F, sep="\t")

reactome = pathfindR::run_pathfindR(input, gene_sets = "Reactome", plot_enrichment_chart = F, list_active_snw_genes = T, n_processes=6, iterations=100)
write.table(reactome, "/data/github/pca_network/results/reactome.txt", quote = F, row.names = F, sep="\t")

go = pathfindR::run_pathfindR(input, gene_sets = "GO-All", plot_enrichment_chart = F, list_active_snw_genes = T, n_processes=6, iterations=100)
...
res = load("/data/github/pca_network/results/pathfindr0624.RData")

go <- go %>% filter(occurrence>99)
melt = melt[which(melt$occurrence>99),]

# network plot for manuscript
pdf("/data/github/pca_network/results/pathfindR/network.pdf", width=22, height=16)
melt = cluster_enriched_terms(go, plot_clusters_graph = T, use_description = T, use_active_snw_genes = T, vertex.label.cex=1, vertex.size.scaling=1.5)
dev.off()
