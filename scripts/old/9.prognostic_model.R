...
# give glmnet a go
y = univ_mat[,c(257,258)]
colnames(y) <- c("time", "status")
y <- as.matrix(y)
x = as.matrix(univ_mat[,c(1:256)])
## Lasso Cox model

# Lasso Cox by using cv.glmnet to obtain the 5-fold CV optimal lambda.min or lambda.1se
set.seed(123)
cvfit <- glmnet::cv.glmnet(x, y, family = "cox", nfolds = 5)
...
## norm shit
x = as.matrix((univ_mat[,1:256]))
y$bcr_status = as.numeric(as.character(y$bcr_status))
colnames(y) <- c("time", "status")
y = as.matrix(y)
set.seed(123)
...
#devtools::source_url("https://github.com/danielegiardiello/Prediction_performance_survival/blob/main/Functions/internal_cv.R")
#int_val <- bootstrap_cv(db = univ_mat, 
#                        B = 10,
#                        time = "days_to_follow_up",
#                        status = "bcr_status",
#                        formula_model = "Surv(days_to_follow_up, bcr_status) ~ REG4 + CTHRC1 + SLC2A4 + JAG2",
#                        formula_ipcw = "Surv(days_to_follow_up, bcr_status) ~ 1",
#                        pred.time = 4000)
