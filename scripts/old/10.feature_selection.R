...
## feature selection methods, check overlaps between methods! increase stringency where applicable too. 

## NB Takes ~6 hours to run.

rf_mat <- mat %>% dplyr::select(-"study")
rf_imp <- rfsrc(Surv(days_to_follow_up, bcr_status) ~., rf_mat, ntree = 1000, nsplit = 0, mtry = ceiling(sqrt(ncol(mat))), nodesize = 15, importance = TRUE)
rf_imp_sample <- subsample(rf_imp, B = 100, subratio = 0.1)
rf_imp_stats <- plot(rf_imp_sample)$stats
rf_importance <- colnames(rf_imp_stats)
rf_importance <- unique(c(rf_importance))

## Random Forest Minimal Depth
rf_md <- var.select(Surv(days_to_follow_up, bcr_status) ~ ., rf_mat, conservative = "high", method = "md", mtry = ceiling(sqrt(ncol(mat))), ntree = 1000, nsplit = 0, nodesize = 15, splitrule = "logrank")
rf_minimal_depth <- rf_md$topvars
rf_minimal_depth <- unique(c(rf_minimal_depth))

## Random Forest Variable Hunting
rf_vh <- var.select(Surv(days_to_follow_up, bcr_status) ~ ., rf_mat, conservative = "high", method = "vh", ntree = 1000, nrep = 100, nsplit = 0, K = 9, nodesize = 15, nstep =1, splitrule = "logrank")
rf_variable_hunter <- rf_vh$topvars
rf_variable_hunter <- unique(c(rf_variable_hunter))

rds_list = list(rf_imp = rf_imp, rf_vh = rf_vh, rf_md = rf_md, rf_importance = rf_importance, rf_variable_hunter = rf_variable_hunter, rf_minimal_depth = rf_minimal_depth)
saveRDS(rds_list, file = "/data/github/pca_network/results/RF_variable_selection.RDS")

# read results instead of running
rf <- readRDS("/data/github/pca_network/results/RF_variable_selection.RDS")
list2env(rf, envir = .GlobalEnv)

#predictors <- Reduce(intersect, list(rf_importance, rf_variable_hunter, rf_minimal_depth))
predictors <- c(rf_importance, rf_minimal_depth, rf_variable_hunter)
predictors <- unique(predictors)
#predictors <- table(predictors)
#predictors <- names(predictors[predictors >= 3])

## Pass these genes to LASSO, Elastic Net and Coxboost
## ensure each fold represents the individual studies.
keep_patient <- rownames(mat)[which(mat$days_to_follow_up != 0)]
y <- as.matrix(mat[keep_patient, c("days_to_follow_up", "bcr_status")])
colnames(y) <- c("time", "status") 
all_x <- model.matrix( ~ 0 + ., mat[keep_patient, c(network_genes, "gleason_score", "preop_psa")])
#all_x <- model.matrix( ~ 0 + ., mat[keep_patient, c(predictors)])

## use "study" as cross validation for lasso and elastic.
group_identifier <- as.numeric(as.factor(mat$study[which(rownames(mat) %in% keep_patient)]))

## LASSO
set.seed(123789)
lambda_max <- max(glmnet(all_x, y, family = "cox", alpha = 1)$lambda)
lambda_seq <- exp(seq(log(lambda_max), log(lambda_max / 100), length = 100))
lasso_fit <- glmnet::cv.glmnet(all_x, y, family = "cox", alpha = 1, lambda = lambda_seq, foldid = group_identifier, type.measure = "C")
lasso_preds <- coef(lasso_fit$glmnet.fit, s=lasso_fit$lambda.min)[,1]
#lasso_preds <- names(lasso_preds[lasso_preds != 0])
...
predictors <- names(lasso_preds)
# assess schoenfeld residuals of the fit cph model
fit_zph <- cox.zph(fit)
remove <- rownames(fit_zph$table)[which(fit_zph$table[,3] < 0.05)]
# if "GLOBAL" is in the vector, remove it using if else
if("GLOBAL" %in% remove){
  remove <- remove[1:length(remove)-1]
}
# remove genes present in remove from lasso_preds
...
fit_zph <- cox.zph(coxph(fx, data = mat, init = as.numeric(lasso_preds), iter.max = 0))
remove <- rownames(fit_zph$table)[which(fit_zph$table[,3] < 0.05)]
coxph_fit <- coxph(fx, data = mat, init = as.numeric(lasso_preds), iter.max = 0)


## Elastic Net - takes time
...
for (i in 1:length(alpha_seq)) {
  fitEN[[i]] <- cv.glmnet(all_x, y, family = "cox", alpha = alpha_seq[i], lambda = lambda_seq, foldid = group_identifier)
}
...
elastic_fit <- fitEN[[idx]]
elastic_preds <- coef(elastic_fit$glmnet.fit, s=elastic_fit$lambda.min)[,1]
elastic_preds <- names(elastic_preds[elastic_preds != 0])

## Coxboost - takes more time.. 
unique_folds <- unique(group_identifier)
folds_list <- lapply(unique_folds, function(fold) {
  which(group_identifier == fold)
})
snowfall::sfInit(parallel=TRUE, cpus=6)
cb.pen <- optimCoxBoostPenalty(time=y[,1],status=y[,2],x=all_x,minstepno=50,maxstepno=200,start.penalty=9*sum(y[,2]==1),iter.max=10,upper.margin=0.05,parallel=TRUE)
#cv.res <- cv.CoxBoost(time=y[,1], status=y[,2],x=all_x,maxstepno=100,parallel=TRUE,K=9,folds=folds_list,type="naive")
cb_fit <- CoxBoost(time=y[,1], status=y[,2], x=all_x, stepno=cb.pen$cv.res$optimal.step, penalty=cb.pen$penalty)

coxboost_preds <- coef(cb_fit)
coxboost_preds <- names(coxboost_preds[coxboost_preds != 0])

#`predictors <- Reduce(intersect, list(coxboost_preds, lasso_preds, elastic_preds))
predictors <- c(coxboost_preds, lasso_preds, elastic_preds, rf_importance, rf_minimal_depth, rf_variable_hunter)
pred_tab <- table(predictors)
predictors <- names(pred_tab[pred_tab >= 5])
predictors <- predictors[!grepl("study", predictors)]

## Check that none violate Prop Hazards
check_pred <- mat[, c(predictors, "days_to_follow_up", "bcr_status")]
fit_ph <- coxph(Surv(days_to_follow_up, bcr_status) ~ ., data = check_pred)
zph <- cox.zph(fit_ph)
remove <- rownames(zph$table)[which(zph$table[,3] < 0.05)]
# GLOBAL
remove <- remove[1:length(remove)-1]

predictors <- predictors[!(predictors %in% remove)]
check_pred <- mat[, c(predictors, "days_to_follow_up", "bcr_status")]
fit_ph <- coxph(Surv(days_to_follow_up, bcr_status) ~ ., data = check_pred, init = as.numeric(lasso_preds), iter.max=0)
zph <- cox.zph(fit_ph)
remove <- rownames(zph$table)[which(zph$table[,3] < 0.05)]
...
## global test is not significant, optional to remove SLC2A6
## now filter to remove genes with confidence intervals crossing 1
fit <- coxph(Surv(days_to_follow_up, bcr_status) ~ ., check_pred)
sjPlot::plot_model(fit)
keep <- rownames(summary(fit)$coef[summary(fit)$coef[, 5] <= 0.05, ])

check_pred <- check_pred[, c(keep, "days_to_follow_up", "bcr_status")]
fit <- coxph(Surv(days_to_follow_up, bcr_status) ~ ., check_pred)
sjPlot::plot_model(fit)

saveRDS(object = list(predictors = keep), file = "/data/github/pca_network/results/predictors.RDS")
...
for(i in unique(mat$study)){
  ...
  fx <- formula(paste("Surv(days_to_follow_up, bcr_status) ~ ", paste(gene_mask, collapse = " + "), " + preop_psa + gleason_score + (1|study)"))
  dev_fit <- coxme::coxme(fx, dev_mat)
...
forest_plot("lasso_1", "Slope Coef", "Slope LCI", "Slope UCI")
forest_plot("lasso_4", "Harrells C", "Harrells C LCI", "Harrells C UCI")
