...
gs <- factor(sml)
groups <- make.names(c("Clone1","Control"))
levels(gs) <- groups
gset$group <- gs
...
clone1_tt <- subset(tT, rownames(tT) %in% clone1_circs)
write.table(clone1_tt, "/data/github/pca_network/results/GSE118959_Clone1_limma.txt", sep="\t", row.names = F, quote=F)
