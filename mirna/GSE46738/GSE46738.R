...
gset <- getGEO("GSE46738", GSEMatrix =TRUE, getGPL=FALSE)
...
meta = data.frame(row.names = colnames(ex), 
                  status = factor(c(rep("PCa", 53), rep("Control", 4))))

rownames(meta) == colnames(ex)
...
library(ggplot2)
pdf("/data/github/pca_network/mirna/GSE46738/GSE46738_PCA.pdf", height=4, width=5)
